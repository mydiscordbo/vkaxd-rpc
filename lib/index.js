...
if (self.timeout) {
  ...
  const errorMessage = 'Vkax JSON-RPC: ';
  ...
  const req = this.protocol.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        const exceededError = new Error(`Vkax JSON-RPC: ${buf.toString('utf8')}`);
...
// For definitions of RPC calls, see various files in: https://github.com/vkaxpay/vkax/tree/master/src
RpcClient.callspec = {
